...
def connect_google_drive(project_name):
    ...
    if not os.path.exists(model_dir):
        ...
        print("You'll find the project on Google Drive, on the folder mrcnn/{} .".format(project_name))
    else:
...
def create_mrcnn_output_directory(project_name):
    ...
    if not os.path.exists(model_dir):
        os.makedirs(model_dir)
        print("New project created {}".format(project_name))
        print("You'll find the project on Google Drive, mrcnn/{} .".format(project_name))
    else:
        print("Project {} already exists. Editing existing project.".format(project_name))
    return model_dir
...
def model_evaluation(dataset_val, test_model, inference_config):
    ...
    for image_id in dataset_val.image_ids:
        ...
        APs.append(AP)
        IoUs.extend(overlaps)

    # Create more points for a smoother curve
    xnew = np.linspace(min(recalls), max(recalls), 300) 

    # Spline interpolation
    spl = make_interp_spline(recalls, precisions, k=3)  # k is the degree of the spline
    smooth_precisions = spl(xnew)

    # Plot Precision-Recall curve
    plt.figure(figsize=(6, 6))
    plt.plot(xnew, smooth_precisions, marker='.')
    plt.title("Precision-Recall curve")
    plt.xlabel("Recall")
    plt.ylabel("Precision")
    plt.savefig('precision_recall_curve.png')  # Save the plot as a file
    plt.show()

    # Plot IoU histogram
    plt.figure(figsize=(6, 6))
    plt.hist(IoUs, bins=10, range=(0,1), linewidth=3)
    plt.title("IoU Distribution")
    plt.xlabel("IoU")
    plt.ylabel("Frequency")
    plt.savefig('iou_distribution.png')  # Save the plot as a file
    plt.show()

    # Plot AP Distribution
    plt.figure(figsize=(6, 6))
    plt.hist(APs, bins=20, alpha=0.7, label='AP')
    plt.xlabel('Average Precision')
    plt.ylabel('Frequency')
    plt.title('AP Distribution')
    plt.legend()
    plt.savefig('ap_distribution.png')  # Save the plot as a file
    plt.show()
